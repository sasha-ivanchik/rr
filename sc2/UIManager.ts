...
class UIManager {
    ...
    async function clickButton(
        ...
        buttonname: string,
        index?: number
    ): Promise<[boolean, Locator | null]> {
        try {
            // Ищем кнопку по тексту или содержимому
            const buttonXPath = `
                //button[contains(text(), '${buttonname}')] |
                //button//*[contains(text(), '${buttonname}')] |
                //*[@role='button']//*[contains(text(), '${buttonname}')]
            `;
    
            let guiObject: Locator | null = page.locator(buttonXPath);
    
            // Если кнопка найдена
            if (await guiObject.count() > 0) {
                if (index !== undefined && (await guiObject.count()) > index) {
                    // Кликаем на кнопку с указанным индексом
                    await guiObject.nth(index).click();
                } else if (!index) {
                    // Кликаем на первую кнопку
                    await guiObject.first().click();
                }
    
                console.log(`Clicked on button: ${buttonname}`);
                return [true, guiObject];
            }
    
            // Если кнопка не найдена, ищем через process_parent
            guiObject = await process_parent(page, buttonname, "Button", index);
            if (guiObject) {
            ...
            console.log(`Not able to find button: ${buttonname}`);
            return [false, null];
        } catch (error) {
            console.log(`Error clicking button: ${buttonname}`, error);
            return [false, null];
        }
    }
